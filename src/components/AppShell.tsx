...
import { IconApps, IconBook, IconMenuFold, IconMenuUnfold, IconMessage, IconRobot, IconSettings } from '@arco-design/web-react/icon';
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const selected = useMemo(() => location.pathname.startsWith('/inbox') ? 'inbox' : location.pathname.startsWith('/agents') ? 'agent-management' : location.pathname.startsWith('/knowledge') ? 'knowledge' : location.pathname.startsWith('/workflows') ? 'workflows' : location.pathname.startsWith('/channels') ? 'channels' : location.pathname.startsWith('/settings') ? 'settings' : '', [location.pathname]);
    ...
    <Layout>
      <Sider width={200} collapsed={collapsed} className="app-sider" breakpoint="md" collapsedWidth={56} onCollapse={setCollapsed}>
        <div className="sider-inner">
          <Menu className="app-menu" mode="vertical" defaultOpenKeys={['resource-center']} selectedKeys={[selected]} onClickMenuItem={(key) => navigate(key === 'inbox' ? '/inbox' : key === 'agent-management' ? '/agents' : key === 'knowledge' ? '/knowledge' : key === 'workflows' ? '/workflows' : key === 'settings' ? '/settings' : '/channels')}>
