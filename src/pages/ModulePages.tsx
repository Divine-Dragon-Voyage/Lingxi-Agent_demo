...
const liveChatLogoMark = `${import.meta.env.BASE_URL}assets/livechat-logo-mark.png`;
...
function ChannelWizard({ open, onClose, channel }: { open: boolean; onClose: () => void; channel?: Channel }) {
  ...
  return <Modal open={open} title={channel ? '配置 LiveChat 渠道' : '新建 LiveChat 渠道'} width={760} onCancel={close} footer={null} afterOpenChange={(visible) => { if (visible) { setStep(0); setChecking(false); setExpanded({ groups: false, reception: false, human: false }); setHumanEnabled(channel?.humanEnabled ?? true); setHotQuestions(channel?.hotQuestions || []); form.setFieldsValue(initialValues); } }}><div className="channel-steps">{steps.map((label, index) => <div className={`${index === step ? 'active' : ''}${index < step ? ' complete' : ''}`} key={label}><span>{index < step ? '✓' : index + 1}</span>{label}</div>)}</div><Form form={form} layout="vertical" initialValues={initialValues}><div className="channel-step-content">{step === 0 && <><Form.Item name="name" label="渠道名称" rules={[{ required: true, message: '请输入渠道名称' }]}><Input /></Form.Item><Form.Item name="agentId" label="接待 AI Agent" rules={[{ required: true, message: '请选择接待 AI Agent' }]}><Select options={agents.map((agent) => ({ label: agent.name, value: agent.id }))} /></Form.Item></>}{step === 1 && <><Form.Item name="accountId" label="Account ID" rules={[{ required: true, message: '请输入 Account ID' }]}><Input /></Form.Item><Form.Item name="accessToken" label="Access Token" rules={[{ required: true, message: '请输入 Access Token' }]}><Input.Password /></Form.Item></>}{step === 2 && <div className="channel-service-config">{section('groups', 'Group 配置', <><Form.Item name="receiveGroups" label="接待 Group" rules={[{ required: true, message: '请选择接待 Group' }]}><GroupMultiSelect placeholder="请选择接待 Group" /></Form.Item>{humanEnabled && <Form.Item name="humanGroups" label="转人工 Group"><GroupMultiSelect placeholder="请选择转人工 Group" /></Form.Item>}</>)}{section('reception', '接待配置', <><Form.Item name="opening" label="开场白"><Input.TextArea rows={3} /></Form.Item><Form.Item label="热门问题"><HotQuestionsEditor value={hotQuestions} onChange={setHotQuestions} /></Form.Item><Form.Item name="ending" label="结束语"><Input.TextArea rows={3} /></Form.Item></>)}{section('human', '转人工', <><div className="channel-inline-switch"><span>转人工开关</span><Switch checked={humanEnabled} onChange={setHumanEnabled} /></div>{humanEnabled && <Form.Item name="humanFallback" label="转人工失败话术"><Input.TextArea rows={3} /></Form.Item>}</>)}<div className="channel-inline-switch channel-status-switch"><span>渠道状态</span><Form.Item name="enabled" valuePropName="checked" noStyle><Switch /></Form.Item></div></div>}</div></Form><div className="channel-step-actions"><Button disabled={step === 0 || checking} onClick={() => setStep((value) => value - 1)}>上一步</Button>{step < 2 ? <Button type="primary" loading={checking} onClick={next}>下一步</Button> : <Button type="primary" onClick={save}>保存</Button>}</div></Modal>;
}
